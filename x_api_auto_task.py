...
（每个主题标题之前空一行，直接输出 主题标题：副标题，不要加编号）
🔥 [主题标题：副标题]（标题字体加粗）
💡 叙事转向：[一句话核心判断。直接写纯文本，禁止在行首使用 > 或 # 符号 只筛选重要发言 3-5 个，如果重要发言不够可以不呈现]
🗣️ **@账号名**|身份标签（如果不明则 |身份标签 都不输出 ） 
  > 具体行为 + 解读（不超过 60 字）
🗣️ **@账号名**|身份标签（如果不明则 |身份标签 都不输出 ） 
  > 具体行为 + 解读（不超过 60 字）
🗣️ **@账号名**|身份标签（如果不明则 |身份标签 都不输出 ） 
  > 具体行为 + 解读（不超过 60 字）
**🔥 核心共识**：[如果没有共识，可以整行删除不输出 直接输出观点文本，绝对禁止在开头添加 - 或 * 符号，"核心共识"4个字必须保持加粗的格式]
**⚔️ 最大分歧**：[如果没有分歧，可以整行删除不输出 直接输出观点文本，绝对禁止在开头添加 - 或 * 符号，"最大分歧"4个字必须保持加粗的格式]

（每个主题标题之前空一行，直接输出 主题标题：副标题，不要加编号）
🤖 [主题标题：副标题]（标题字体加粗加大）
💡 叙事转向：[一句话核心判断。直接写纯文本，禁止在行首使用 > 或 # 符号 只筛选重要发言 3-5 个，如果重要发言不够可以不呈现]
🗣️ **@账号名**|身份标签（如果不明则写热心网友） 
  > 具体行为 + 解读（不超过 60 字）
🗣️ **@账号名**|身份标签（如果不明则写热心网友）
  > 具体行为 + 解读（不超过 60 字）
🗣️ **@账号名**|身份标签 （如果不明则写热心网友）
...
（每个主题之间空一行，emoji 可根据主题自由搭配 包括但不限于🤖⚔️🏭🦾💡🔥📊🚀💰🌊）
